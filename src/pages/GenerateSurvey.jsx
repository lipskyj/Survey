...
Do not wrap in SURVEY_JSON, SURVEY_CONTENT, or any other key. Only scale_questions and open_questions at the root level.`;
  ...
  if (bgQuestions.include_class) {
    ...
    const gradeChoices = selectedGradeRanges.flatMap(range => gradeChoicesMap[range] || []);
    const fallbackGrades = gradeChoicesMap.middle;

    const classNumberChoices = Array.from({ length: 9 }, (_, i) => ({
      value: `${i + 1}`,
      label: `${i + 1}`
    }));

    // Question 1: which grade level
    questionsToCreate.push({
      survey_id: newSurveyId, order_index: orderIndex++, question_type: 'single_choice', kit_domain: 'none',
      ...
      is_required: true,
      choices: gradeChoices.length > 0 ? gradeChoices : fallbackGrades,
      is_generated: true
    });

    // Question 2: which class number
    questionsToCreate.push({
      survey_id: newSurveyId, order_index: orderIndex++, question_type: 'single_choice', kit_domain: 'none',
      prompt_hebrew: isArabic
        ? (survey.audience === 'parents' ? 'ما رقم صف ابنك/ابنتك؟' : 'ما رقم صفك؟')
        : (survey.audience === 'parents' ? 'מה מספר הכיתה של ילדך/ילדתך?' : 'מה מספר הכיתה שלך?'),
      is_required: true,
      choices: classNumberChoices,
      is_generated: true
    });
  }
